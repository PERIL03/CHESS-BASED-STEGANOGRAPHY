...
# Optional secret for session/security hardening
if os.getenv('SECRET_KEY'):
    app.config['SECRET_KEY'] = os.getenv('SECRET_KEY')

# X402 micropayment configuration (from env)
...
def initiate_payment():
    ...
    try:
        ...
        # X402 micropayment amounts (in wei) from environment
        payment_amounts = {
            "encode": X402_ENCODE_PRICE,
            "decode": X402_DECODE_PRICE
        }
        ...
        return jsonify({
            ...
            "operation": operation,
            "payment_address": PAYMENT_WALLET_ADDRESS,  # X402 payment address
...
def list_pgns():
    """List PGN files for a given wallet address"""
    try:
        wallet_address = request.args.get('wallet_address')
        if not wallet_address:
            return jsonify({"success": False, "error": "wallet_address query param required"}), 400

        results = []
        for data in pgn_metadata_store.values():
            if data.get("wallet_address") == wallet_address:
                results.append({
                    "pgn_id": data.get("pgn_id"),
                    "file_type": data.get("file_type"),
                    "file_size": data.get("file_size"),
                    "created_at": data.get("created_at"),
                    "original_filename": data.get("original_filename", "secret.bin"),
                    "download_url": f"/api/download/{data.get('pgn_id')}"
                })

        # Sort newest first
        results.sort(key=lambda x: x["created_at"], reverse=True)
        return jsonify({"success": True, "pgns": results})

    except Exception as e:
        app.logger.error(f"PGN list error: {str(e)}")
        return jsonify({"success": False, "error": "Failed to list PGNs"}), 500

@app.route("/api/payment/status/<transaction_id>", methods=["GET"])
def payment_status(transaction_id):
    """Get status for a specific payment transaction"""
    try:
        ...
        return jsonify({
            ...
            "tx_hash": tx.get("tx_hash", None)
        })

    except Exception as e:
        app.logger.error(f"Payment status error: {str(e)}")
        return jsonify({"success": False, "error": "Failed to get payment status"}), 500
